...
def generate_rnode_data() -> None:
    ...
    with generate_cmd_opts() as command_line_options ,\
        ...
        transfer_funds(context, ceremony_master, CEREMONY_MASTER_PRIVATE.get_public_key().get_vault_address(), VALIDATOR_A_PRIVATE.get_public_key().get_vault_address(), 100, CEREMONY_MASTER_PRIVATE, 100000, 1)
        transfer_funds(context, ceremony_master, VALIDATOR_A_PRIVATE.get_public_key().get_vault_address(), VALIDATOR_B_PRIVATE.get_public_key().get_vault_address(), 200, VALIDATOR_A_PRIVATE, 100000, 1)
        transfer_funds(context, ceremony_master, VALIDATOR_B_PRIVATE.get_public_key().get_vault_address(), CEREMONY_MASTER_PRIVATE.get_public_key().get_vault_address(), 300, VALIDATOR_B_PRIVATE, 100000, 1)
