...
def start_node_with_heartbeat(
    ...
    cli_options = {
        "--heartbeat-check-interval": f"{heartbeat_check_interval}sec",
        "--heartbeat-max-lfb-age": f"{heartbeat_max_lfb_age}sec",
...
def test_heartbeat_disabled_by_config(
    ...
    """
    Test that heartbeat is disabled when --heartbeat-enabled=false.
