...
def pytest_addoption(parser: Parser) -> None:
    parser.addoption("--startup-timeout", type=int, action="store", default=60 * 5, help="timeout in seconds for starting a node")
    parser.addoption("--converge-timeout", type=int, action="store", default=60 * 5, help="timeout in seconds for network converge")
...
def make_wallets_file_lines(wallet_balance_from_private_key: Dict[PrivateKey, int]) -> List[str]:
    ...
    for private_key, token_amount in wallet_balance_from_private_key.items():
        line = f'{private_key.get_public_key().get_vault_address()},{token_amount},0'
        result.append(line)
    return result
