...
def test_backward_compatible(command_line_options: CommandLineOptions, random_generator: Random, docker_client: DockerClient) -> None:
    ...
    with  testing_context(command_line_options, random_generator, docker_client) as context, \
        ...
        get_vault_balance(context, bootstrap_node, CEREMONY_MASTER_PRIVATE.get_public_key().get_vault_address(), CEREMONY_MASTER_PRIVATE, 100000, 1)
        get_vault_balance(context, validator_a, VALIDATOR_A_PRIVATE.get_public_key().get_vault_address(), VALIDATOR_A_PRIVATE, 100000, 1)
        get_vault_balance(context, validator_b, VALIDATOR_B_PRIVATE.get_public_key().get_vault_address(), VALIDATOR_B_PRIVATE, 100000, 1)
