    ...
    K: Clone + Debug + Default + Send + Sync + Serialize + for<'a> Deserialize<'a> + 'static,
{
    ...
    pub fn create_checkpoint(&self) -> Result<Checkpoint, RSpaceError> {
        ...
        self.soft_report.lock().expect("soft_report lock in create_checkpoint").clear();
        self.report.lock().expect("report lock in create_checkpoint").clear();
    ...
    K: Clone + Debug + Default + Send + Sync + Serialize + for<'a> Deserialize<'a> + 'static,
{
    ...
    fn create_soft_checkpoint(&self) -> SoftCheckpoint<C, P, A, K> {
        ...
        ReportingRspace::create_soft_checkpoint(self).expect("create_soft_checkpoint failed in ReportingRspace")
    }
