...
/**
  ...
  */
final case class RSpacePlusPlusStateChange(
...
object RSpacePlusPlusStateChange {
  ...
  def apply[F[_]: Concurrent, C, P, A, K](
      preStateReader: RSpacePlusPlusHistoryReaderBinary[F, C, P, A, K],
      postStateReader: RSpacePlusPlusHistoryReaderBinary[F, C, P, A, K],
      eventLogIndex: EventLogIndex,
      serializeC: Serialize[C]
  ): F[RSpacePlusPlusStateChange] =
    for {
      ...
      _ <- new Exception("State change compute logic error: empty channel change for produce.")
            .raiseError[F, RSpacePlusPlusStateChange]
            .whenA(
              produceChanges
    ...
    } yield RSpacePlusPlusStateChange(produceChanges, consumeChanges, joinsMap)

  def empty: RSpacePlusPlusStateChange = RSpacePlusPlusStateChange(Map.empty, Map.empty, Map.empty)
  def combine(
      x: RSpacePlusPlusStateChange,
      y: RSpacePlusPlusStateChange
    ...
    RSpacePlusPlusStateChange(newDC, newKC, newJ)
  }

  implicit def monoid: Monoid[RSpacePlusPlusStateChange] =
    new Monoid[RSpacePlusPlusStateChange] {
      def empty: RSpacePlusPlusStateChange = RSpacePlusPlusStateChange.empty
      def combine(
          x: RSpacePlusPlusStateChange,
          y: RSpacePlusPlusStateChange
      ): RSpacePlusPlusStateChange =
        RSpacePlusPlusStateChange.combine(x, y)
    }
}
