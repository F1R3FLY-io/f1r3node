...
fn contracts() -> Vec<(String, i64)> {
    vec![
      ...
      // Cost is 350 (not 342) because receives-first evaluation makes this
      // COMM produce-triggered: consume stores continuation, then persistent
      // produce fires COMM + re-produces (8 extra for re-produce upfront).
      ...
      // Cost changed from 406 to 334 with per-bind peeks (BTreeSet<i32>
      // instead of bool). Peek is now tracked per-bind, changing the
      // consume's cost accounting path.
      (String::from("@0!(0) | for (_ <<- @0) { 0 }"), 334i64),
