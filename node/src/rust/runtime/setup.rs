...
> {
    ...
    let casper_launch = {
        ...
        Arc::new(casper::rust::engine::casper_launch::CasperLaunchImpl::new(
            ...
            event_publisher,
            Arc::new(block_retriever.clone()),
            Arc::new(engine_cell.clone()),
            Arc::new(Mutex::new(Some(block_store.clone()))),
            Arc::new(Mutex::new(Some(block_dag_storage.clone()))),
            Arc::new(Mutex::new(Some(deploy_storage))),
            Arc::new(Mutex::new(Some(casper_buffer_storage.clone()))),
            Arc::new(Mutex::new(Some(rspace_state_manager))),
            Arc::new(tokio::sync::Mutex::new(runtime_manager.clone())),
            Arc::new(Mutex::new(Some(estimator.clone()))),
            // Explicit parameters
