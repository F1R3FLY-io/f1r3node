...
impl Validate {
    ...
    pub async fn block_summary(
        ...
        tracing::debug!(target: "rchain.casper", "before-block-hash-validation");
        match Self::block_hash(block) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-timestamp-validation");
        match Self::timestamp(block, block_store) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-shard-identifier-validation");
        match Self::shard_identifier(block, shard_id) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-deploys-shard-identifier-validation");
        match Self::deploys_shard_identifier(block, shard_id) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-repeat-deploy-validation");
        match Self::repeat_deploy(block, s, block_store, expiration_threshold) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-block-number-validation");
        match Self::block_number(block, s) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-future-transaction-validation");
        match Self::future_transaction(block) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-transaction-expired-validation");
        match Self::transaction_expiration(block, expiration_threshold) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-justification-follows-validation");
        match Self::justification_follows(block, block_store) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-parents-validation");
        match Self::parents(block, genesis, s, estimator).await {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-sequence-number-validation");
        match Self::sequence_number(block, s) {
            Either::Left(err) => return Either::Left(err),
            Either::Right(_) => {}
        }
        tracing::debug!(target: "rchain.casper", "before-justification-regression-validation");
    ...
    pub fn repeat_deploy(
        ...
        tracing::debug!(target: "rchain.casper", "before-repeat-deploy-get-parents");
        ...
        tracing::debug!(target: "rchain.casper", "before-repeat-deploy-duplicate-block");
        ...
        tracing::debug!(target: "rchain.casper", "before-repeat-deploy-duplicate-block-log");
