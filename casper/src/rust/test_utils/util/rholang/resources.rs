...
/// Creates a test store manager with shared scope for ALL stores (blocks, DAG, RSpace, etc.).
///
/// This is the ORIGINAL behavior for casper tests where all nodes in a test network
/// share the same storage scope. This allows nodes to see each other's blocks directly
/// in storage, which is required for proper DAG operations and block propagation.
///
/// Use this for casper tests that need nodes to share block/DAG storage.
/// Use `mk_test_rnode_store_manager_with_dual_scope` for transaction API tests that
/// need isolated block/DAG stores but shared RSpace.
#[cfg(feature = "test-utils")]
pub fn mk_test_rnode_store_manager_shared_all_stores(
    scope_id: String,
) -> Box<dyn KeyValueStoreManager> {
    let (shared_path, _temp_dir) = &*SHARED_LMDB_ENV;
    // Use the same scope for ALL stores (blocks, DAG, RSpace, etc.)
    // This matches the original behavior where nodes share storage
    Box::new(mk_test_rnode_store_manager_with_scope(
        shared_path.clone(),
        Some(scope_id),
    ))
...
pub fn mk_test_rnode_store_manager_with_dual_scope(
    ...
    Box::new(LmdbDirStoreManager::new(
        shared_path.clone(),
        db_mappings.into_iter().collect(),
    ))
}
...
pub async fn mk_test_rnode_store_manager_with_shared_rspace(
    ...
    // mk_test_rnode_store_manager_with_dual_scope already returns Box<dyn KeyValueStoreManager>
    let mut new_kvm = mk_test_rnode_store_manager_with_dual_scope(
        new_node_scope,
...
/// Helper function to create KeyValueBlockStore from dyn KeyValueStoreManager
pub async fn block_store_from_dyn(
    kvm: &mut dyn KeyValueStoreManager,
) -> Result<
...
> {
    ...
    let store = kvm.store("blocks".to_string()).await.map_err(|e| {
        shared::rust::store::key_value_store::KvStoreError::IoError(format!(
            "Failed to get blocks store: {:?}",
            e
        ))
    })?;
    let store_approved_block = kvm
        .store("blocks-approved".to_string())
        .await
        .map_err(|e| {
            shared::rust::store::key_value_store::KvStoreError::IoError(format!(
                "Failed to get blocks-approved store: {:?}",
                e
            ))
        })?;

    Ok(KeyValueBlockStore::new(store, store_approved_block))
}

/// Helper function to create KeyValueDeployStorage from dyn KeyValueStoreManager
pub async fn deploy_storage_from_dyn(
    kvm: &mut dyn KeyValueStoreManager,
) -> Result<
...
/// Helper function to create CasperBufferKeyValueStorage from dyn KeyValueStoreManager
pub async fn casper_buffer_storage_from_dyn_kvm(
    kvm: &mut dyn KeyValueStoreManager,
) -> Result<
    block_storage::rust::casperbuffer::casper_buffer_key_value_storage::CasperBufferKeyValueStorage,
    shared::rust::store::key_value_store::KvStoreError,
> {
    // Use the existing casper_buffer_storage_from_dyn function
    casper_buffer_storage_from_dyn(kvm).await
}
