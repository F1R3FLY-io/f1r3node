...
object StandardDeploys {
  ...
  val systemVaultPk         = "27e5718bf55dd673cc09f13c2bcf12ed7949b178aef5dcb6cd492ad422d05e9d"
  val multiSigSystemVaultPk = "2a2eaa76d6fea9f502629e32b0f8eea19b9de8e2188ec0d589fcafa98fb1f031"
  val poSGeneratorPk        = "a9585a0687761139ab3587a4938fb5ab9fcba675c79fefba889859674046d4a5"
  val vaultsGeneratorPk     = "a06959868e39bb3a8502846686a23119716ecd001700baf9e2ecfa0dbf1a3247"
  val stackPk               = "c94e647de6876c954ebb7b64c40a220227770f9be003635edfe3336a1a2c8605"
  val fileRegistryPk        = "78ea7f9a176eec0bf7f4fb4ef1020ae3b8ebd9fa80c1337c70e0ec911f6cca31"
  ...
  val (systemVaultPubKey, systemVaultTimestamp) = (toPublic(systemVaultPk), 1559156183943L)
  val (multiSigSystemVaultPubKey, multiSigSystemVaultTimestamp) =
    (toPublic(multiSigSystemVaultPk), 1571408470880L)
  val (poSGeneratorPubKey, poSGeneratorTimestamp) = (toPublic(poSGeneratorPk), 1559156420651L)
  val vaultsGeneratorPubKey: PublicKey            = toPublic(vaultsGeneratorPk)
  val (stackPubKey, stackTimestamp)               = (toPublic(stackPk), 1751539590099L)
  val (fileRegistryPubKey, fileRegistryTimestamp) = (toPublic(fileRegistryPk), 1772036778848L)
  ...
  val systemPublicKeys: Seq[PublicKey] = Seq(
    ...
    authKeyPubKey,
    systemVaultPubKey,
    multiSigSystemVaultPubKey,
    poSGeneratorPubKey,
    vaultsGeneratorPubKey,
    stackPubKey,
    fileRegistryPubKey
  )
  ...
  def systemVault(shardId: String): Signed[DeployData] =
    toDeploy(
      CompiledRholangSource("SystemVault.rho"),
      systemVaultPk,
      systemVaultTimestamp,
      shardId
    )

  def multiSigSystemVault(shardId: String): Signed[DeployData] =
    toDeploy(
      CompiledRholangSource("MultiSigSystemVault.rho"),
      multiSigSystemVaultPk,
      multiSigSystemVaultTimestamp,
      shardId
    )
  ...
  def fileRegistry(shardId: String): Signed[DeployData] =
    toDeploy(
      CompiledRholangSource("FileRegistry.rho"),
      fileRegistryPk,
      fileRegistryTimestamp,
      shardId
    )
  ...
  def vaultsGenerator(
    ...
    toDeploy(
      VaultsGenerator(vaults, supply, isLastBatch),
      vaultsGeneratorPk,
      timestamp,
      shardId
    )
