...
object OrphanFileCleanup {
  ...
  // Matches the strict exact canonical term generated by clients for file uploads:
  // new return, file(`rho:io:file`) in { file!("register", "<hash>", <size>, "<name>", *return) }
  val CanonicalFileDeployPattern =
    """^\s*new\s+[a-zA-Z0-9_]+,\s*[a-zA-Z0-9_]+\(`rho:io:file`\)\s*in\s*\{\s*[a-zA-Z0-9_]+!\(\s*"register"\s*,\s*"([a-f0-9]{64})"\s*,\s*(\d+)\s*,\s*"[^"]*"\s*,\s*\*?[a-zA-Z0-9_]+\s*\)\s*\}\s*$""".r

  /**
    * Checks whether a deploy's term exactly matches the canonical file-registration pattern.
    * This strict regex prevents DA-gating stalls from crafted Rholang comments containing fake hashes.
    */
  def isFileRegistrationDeploy(d: DeployData): Boolean =
    CanonicalFileDeployPattern.findFirstMatchIn(d.term).isDefined

  /**
    * Checks whether a raw Rholang term string matches the canonical file-registration format.
    */
  def isCanonicalFileDeploy(term: String): Boolean =
    CanonicalFileDeployPattern.findFirstMatchIn(term).isDefined

  /**
    * Extracts the 64-char hex file hash from a canonical file-registration deploy term.
    * Returns `None` if the term does not strictly match.
    */
  def extractFileHash(d: DeployData): Option[String] =
    CanonicalFileDeployPattern.findFirstMatchIn(d.term).map(_.group(1))

  /**
    * Extracts the file size from a file-registration deploy term.
    * Returns `None` if the term does not contain a recognizable file size,
    * allowing backpressure systems to properly reject invalid payloads.
    */
  def extractFileSize(d: DeployData): Option[Long] =
    CanonicalFileDeployPattern
      .findFirstMatchIn(d.term)
      .flatMap(m => scala.util.Try(m.group(2).toLong).toOption)
